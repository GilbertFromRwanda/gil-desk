...
**Phase 3 (Go Control Plane), authentication foundation** done: `backend/internal/auth`, HTTP endpoints on `cmd/server`. Real accounts, JWTs, and refresh-token rotation — this covers G-13/G-14/G-15. **Not done**: 2FA/TOTP (G-16), wiring an authenticated user identity into rendezvous's `AuthorizeDevice` (G-17 — right now that RPC has no caller-identity check at all, which is a real gap, not an oversight to gloss over), rate limiting (G-18), audit events (G-19).
